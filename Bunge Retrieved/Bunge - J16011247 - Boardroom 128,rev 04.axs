PROGRAM_NAME='Bunge - J16011247 - Boardroom 128,rev 04'
(***********************************************************)
(*  FILE_LAST_MODIFIED_ON: 11/15/2018  AT: 12:04:37        *)
(***********************************************************)
(*  FILE REVISION: Rev 4                                   *)
(*  REVISION DATE: 07/18/2019  AT: 12:24:14                *)
(*                                                         *)
(*  COMMENTS:                                              *)
(*  MRC:                                                   *)
(*  Added support for additional TP:                       *)
(*  commented out combine page flips                       *)
(*                                                         *)
(***********************************************************)
...
dvTP2               =   10002:01:0      //Ipad Apple Air 2 running Touchpanel Control.  Use TP Transfer or TP Cloud to load files. 
...
INTEGER RESTART_HOUR = 0
INTEGER RESTART_MINUTE = 0

dev dvTPAll[]={dvTP1,dvTP2}  //room can be modified to support three touchpanels for future use
...
DEFINE_FUNCTION                   NAV_select_vid_page() {
    ...
    send_command dvTPAll,"'PPON-0205 Select Video Combined'"
    nTabSelected[1]=3
   // select {
//	active(nCombinedMode==1 and nLastPanel<3) :  send_command dvTPAll,"'PPON-0203 Select Video AB'"
//	active(nCombinedMode==1 and nLastPanel>2) :  send_command dvTPAll,"'PPON-0208 Select Video C'"
//	active(nCombinedMode==2 and nLastPanel<2) :  send_command dvTPAll,"'PPON-0206 Select Video A'"
//	active(nCombinedMode==2 and nLastPanel>1) :  send_command dvTPAll,"'PPON-0204 Select Video BC'"
//	active(nCombinedMode==3) :                   send_command dvTPAll,"'PPON-0205 Select Video Combined'"
//	active(nCombinedMode==4 and nLastPanel==1) : send_command dvTPAll,"'PPON-0206 Select Video A'"
//	active(nCombinedMode==4 and nLastPanel==2) : send_command dvTPAll,"'PPON-0207 Select Video B'"
//	active(nCombinedMode==4 and nLastPanel==3) : send_command dvTPAll,"'PPON-0208 Select Video C'"
//    }
}
...
DEFINE_FUNCTION                   SUBSCRIBE_ALL()
{
    ...
    CALL 'SUBSCRIBE' (DSP_AUTOMIX_CONF,AUTOMIXER,1,0,MUTE) ///FB[25]
    CALL 'SUBSCRIBE' (DSP_AUTOMIX_CONF,AUTOMIXER,2,0,MUTE) ///FB[26]

    CALL 'SUBSCRIBE' (DSP_CONTENT_TO_VC_GAIN,GAIN,1,0,MUTE) ///FB[27]
    CALL 'SUBSCRIBE%'(DSP_PROGRAM_GAIN,N_GAIN,0,1,GAIN) //FB[28]
    CALL 'SUBSCRIBE' (DSP_PROGRAM_GAIN,N_GAIN,0,1,MUTE) ///FB[29]
    CALL 'SUBSCRIBE' (DSP_PROGRAM_GAIN,GAIN,1,0,MUTE) ///FB[30]
...
cSourceNames[04]="'Clickshare 1'"     
cSourceNames[05]="'Clickshare 2'" 
...
TIMELINE_EVENT[1]//Feedback timeline
{
    ...
    // system reboot if current time matches reboot time
//    if((TIME_TO_HOUR(TIME) == RESTART_HOUR) && (TIME_TO_MINUTE(TIME) == RESTART_MINUTE))
//    {
//		if(!nAutoRebootFlag)	// check flag to make sure it only fires once
//		{
//			nAutoRebootFlag = 1;
//			do_push(dvTP1,17)
//		}
...
TIMELINE_EVENT[3]//BSS FEEDBACK TIMELINE
{
    ...
    */
    ...
    MASTEROUT_VOL[09] = TYPE_CAST(MID_STRING(FB[28],10,1)); //program vol
    SEND_LEVEL dvTPAll,09,MASTEROUT_VOL[09];
    [dvTPAll,1127] = FB[29][12];
    [dvTPAll,1130] = FB[30][12];
}
...
BUTTON_EVENT [dvTPAll,1]    //NAV - Begin
{
	PUSH:
	{
		CALL 'SET_VAL'(DSP_VC_GAIN,N_GAIN,0,1,MUTE,0)			// vtc mute
		CALL 'SET_VAL'(DSP_AUTOMIX,AUTOMIXER,1,0,MUTE,0)		// lav in room
		CALL 'SET_VAL'(DSP_AUTOMIX,AUTOMIXER,2,0,MUTE,0) 		// handheld in room
		CALL 'SET_VAL'(DSP_AUTOMIX_CONF,AUTOMIXER,1,0,MUTE,0)	// lav to conference
		CALL 'SET_VAL'(DSP_AUTOMIX_CONF,AUTOMIXER,2,0,MUTE,0)	// handheld to conference
		CALL 'SET_VAL'(DSP_PROGRAM_GAIN,N_GAIN,0,1,MUTE,0)		// program mute
	}
}
...
BUTTON_EVENT [dvTPAll,4] {  //NAV - VC Content Page
	...
	send_command dvTPAll,"'PAGE-002 Main Page'"
	send_command dvTPAll,"'@PPX'"
	send_command dvTPAll,"'PPON-0209 Select VC Content'"
    }
}
...
BUTTON_EVENT [dvTPAll,5] {  //NAV - Audio Routing Page
	...
	send_command dvTPAll,"'PAGE-002 Main Page'"
	send_command dvTPAll,"'@PPX'"
	send_command dvTPAll,"'PPON-0211 Select Audio Group 2'"
...
BUTTON_EVENT [dvTPAll,7] {  //NAV - Display Power Page
	...
	send_command dvTPAll,"'@PPX'"
	select {
	    active(nCombinedMode==1 and nLastPanel<3) :  send_command dvTPAll,"'PPON-0701 Display Power AB'"
	    active(nCombinedMode==1 and nLastPanel>2) :  send_command dvTPAll,"'PPON-0706 Display Power C'"
	    active(nCombinedMode==2 and nLastPanel<2) :  send_command dvTPAll,"'PPON-0704 Display Power A'"
	    active(nCombinedMode==2 and nLastPanel>1) :  send_command dvTPAll,"'PPON-0702 Display Power BC'"
	    active(nCombinedMode==3) :                   send_command dvTPAll,"'PPON-0703 Display Power ABC'"
	    active(nCombinedMode==4 and nLastPanel==1) : send_command dvTPAll,"'PPON-0704 Display Power A'"
	    active(nCombinedMode==4 and nLastPanel==2) : send_command dvTPAll,"'PPON-0705 Display Power B'"
	    active(nCombinedMode==4 and nLastPanel==3) : send_command dvTPAll,"'PPON-0706 Display Power C'"
	}
    }
}
...
BUTTON_EVENT [dvTPAll,017]		//NAV - Shutdown
{
    PUSH:
	{
		nTabSelected[1]=0
		CALL 'SET_VAL'(DSP_VC_GAIN,N_GAIN,0,1,MUTE,0)			// vtc mute
		CALL 'SET_VAL'(DSP_AUTOMIX,AUTOMIXER,1,0,MUTE,1)		// lav in room
		CALL 'SET_VAL'(DSP_AUTOMIX,AUTOMIXER,2,0,MUTE,1) 		// handheld in room
		CALL 'SET_VAL'(DSP_AUTOMIX_CONF,AUTOMIXER,1,0,MUTE,1)	// lav to conference 
		CALL 'SET_VAL'(DSP_AUTOMIX_CONF,AUTOMIXER,2,0,MUTE,1)	// handheld to conference
	...
	//	CALL 'SET_VAL'(DSP_AUTOMIX,AUTOMIXER,5,0,MUTE,1) 
		...
		CALL 'SET_VAL'(DSP_PROGRAM_GAIN,N_GAIN,0,1,MUTE,1)		// program mute
		
		send_level dvMatrixOutAll[1],50,0
		send_level dvMatrixOutAll[2],50,0
		send_level dvMatrixOutAll[5],50,0
		send_level dvMatrixOutAll[6],50,0
		
		off[vdvCiscoSX80,255]; 	// vtc to standby
...
BUTTON_EVENT [dvTPAll,115] {//MATRIX ROUTE VIDEO
	...
	select {
		...
		if(nSourceRouted[6]<>button.input.channel-nOffset) {
		    ...
		    send_command dvTPAll,"'PAGE-003 Audio Confirm'"
		    send_command dvTPAll,"'^txt-51,0,Do you want to listen to',$0A,Nav_ReplaceLIneFeed(cSourceNames[nSourceRoutedTemp[6]]),' in the Room Speakers?'"
		}
	    }
	}
    }
